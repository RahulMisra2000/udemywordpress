...
class Search {
  ...
  getResults() {
    ...
    $.getJSON(universityData.root_url + '/wp-json/university/v1/search?term=' + this.searchField.val(), 
      (results) => {
        // **************** Notice the use of template literal so we don't have to use \ before end of line
        // and use concatenation operator to mix static text with variable values.
        // YOU CAN WRITE ANY js inside the ${} as long as it returns something. Very powerful... you are doing all this inside 
        // the template string
        // *********************************************************************************************************************
        this.resultsDiv.html(`
            <div class="row">
